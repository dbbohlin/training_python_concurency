...
def repeated_task(work_queue):
    ...
    while not work_queue.empty():
        try:
            ...
            logger.debug(f'Exception caught, x = {x}')
            return x / 100
        y = 0
        while y < _work:
            x += 1
...
def testing_thread_queue():
    _interations = [1000, 10001, 100002, 1000003, 1000004]
    _queue = queue.Queue()
    for _ in _interations:
        _queue.put_nowait(_)
    _worker = threading.Thread(target=repeated_task, args=(_queue,))
    _worker.start()
    _worker.join()


def testing_thread_queue_multiple():
    ...
    for _ in _interations:
        _queue.put_nowait(_)
    _worker1 = threading.Thread(target=repeated_task, args=(_queue,))
    _worker2 = threading.Thread(target=repeated_task, args=(_queue,))
    _worker1.start()
    _worker2.start()
